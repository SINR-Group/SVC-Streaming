...
while True:
    ...
    for batch, (crops, ctx_frames, _) in enumerate(train_loader):
        ...
        if just_resumed or train_iter % args.eval_iters == 0 or train_iter == 20000:
            ...
            torch.save(encoder, '{}encoder_layer{}_itrs{}'.format('./model/', layers, train_iter) )
            torch.save(binarizer, '{}binarizer_layer{}_itrs{}'.format('./model/', layers, train_iter) )
            torch.save(unet, '{}unet_layer{}_itrs{}'.format('./model/', layers, train_iter) )
            torch.save(decoder, '{}decoder_layer{}_itrs{}'.format('./model/', layers, train_iter) )
