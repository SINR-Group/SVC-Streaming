...
class DecoderCell2(nn.Module):
    def __init__(self, v_compress, shrink, bits, fuse_level):
        ...
        #self.conv3 = nn.Conv2d(128*10, 512*10, groups=10, kernel_size=1, stride=1, padding=0, bias=False) Mallesh
        self.conv1 = nn.Conv2d(bits, 64, groups=1, kernel_size=1, stride=1, padding=0, bias=False) # groups=10
        self.conv2 = nn.Conv2d(64, 128, groups=1, kernel_size=1, stride=1, padding=0, bias=False)
        self.conv3 = nn.Conv2d(128, 512, groups=1, kernel_size=1, stride=1, padding=0, bias=False)
    ...
    def forward(self, input, hidden1, hidden2, hidden3, hidden4,
        ...
        x = x.reshape(b,1,-1,h,w).sum(1)
        #x = x.reshape(b,10,-1,h,w).sum(1) Mallesh
        hidden1 = self.rnn1(x, hidden1)
