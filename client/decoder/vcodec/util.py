...
def prepare_unet_output(unet, unet_input, flows, warp):
    ...
    # print("########## prepare unet output")
    # print(type(unet_input))
    # print(type(unet))
...
def forward_model(model, cooked_batch, ctx_frames, args, v_compress,
    ...
    (_,_,_,d2_h_1, d2_h_2, d2_h_3, d2_h_4) = init_rnn(batch_size,height,width)
...
def forward_decoder(model, code, flows, ctx_frames, heirarchy):
    d2, unet = model
    # res, _,_, flows = cooked_batch
    ...
    batch_size, _, height, width = ctx_frames.size()
    (encoder_h_1, encoder_h_2, encoder_h_3,
     decoder_h_1, decoder_h_2, decoder_h_3, decoder_h_4) = init_rnn(batch_size,
                                                                      height,
                                                                      width)
    ...
    dec_unet_output1 = Variable(torch.zeros(1,), volatile=True).cuda()
    dec_unet_output2 = Variable(torch.zeros(1,), volatile=True).cuda()
 
    dec_unet_output1, dec_unet_output2 =  prepare_unet_output(
            unet, torch.cat([frame1, frame2], dim=0), flows, warp=True)
    ...
    for jj in range(3 - max_fuse_level):
        dec_unet_output1[jj] = None
        dec_unet_output2[jj] = None
