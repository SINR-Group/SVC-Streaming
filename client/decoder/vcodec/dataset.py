...
def get_group_filenames(filename, img_idx, distance1, distance2):
    """
    gets filenames based on heirarchy
    """
...
class ImageFolder(data.Dataset):
    ...
    def _load_image_list(self):
        """
        loads list of image filenames in self.imgs
        """
        self.imgs = []
        ...
        for filename in glob.iglob(self.root + '/*png'):
            ...
            # check whether the current file and its context frames exist 
            # and then add them to imgs list
    ...
    def get_group_data(self, filename):
        """
        returns a tuple. 
        element1 - original image sandwiched between context frames
        element2 - original filename
        """
        img_idx = int(filename[:-4].split('_')[-1])
    ...
    def __getitem__(self, index):
        ...
        # img is currently cat of img and context frames (9 long)

        if self.args.warp:
            bmv = np.concatenate(get_bmv(
                get_bmv_filenames(self.mv_dir, main_fn)), axis=2)
            ...
            # img is now cat of context frames and frame and bmv
        ...
        if not self.args.warp:
            img = np.float64(img)
            ctx_frames = np.float64(ctx_frames)
        ...
        # print(data.shape, ctx_frames.shape)
